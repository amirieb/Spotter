...
class Model(Container):
    ...
    def _fit_loop_lit(self, f, ins, out_labels=[], batch_size=32,
                  nb_epoch=100, verbose=1, callbacks=[],
                  val_f=None, val_ins=None, shuffle=True, acc_thr = .8,
                  callback_metrics=[]):
        '''Abstract fit function for f(ins).
        Assume that f returns a list, labeled by out_labels.
        ...
        # Returns
            `History` object, Leitner queues, and ranked list of spurious instances in training data. 
        '''
        do_validation = False
        if val_f and val_ins:
            do_validation = True
        ...
        allQs= [index_array.tolist(), [], [] , [], []]# Leitner queues - tracks the current queues of instances
        q0_stat={}# monitors queue_0 of Leitner
        qAll_stat={} # tracks number of instances in queues throughout training process   
        space_pow = 2 # instances in queue i are reviewed at every space_pow^i iterations
         
        for epoch in range(nb_epoch):                              
            q = np.empty(0, dtype=int)            
            for i in range(len(allQs) - 1, -1, -1):
                if ((epoch + 1) % int(pow(space_pow, i))) == 0:
                    q = np.insert(q, len(q), allQs[i])
            q_size = len(q)                    
            if q_size == 0:
                continue
            ...
            epoch_logs = {}
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                batch_ids = q[batch_start:batch_end]
                try:
                    if type(ins[-1]) is float:
                        # do not slice the training phase flag
                        ins_batch = slice_X(ins[:-1], batch_ids) + [ins[-1]]
            ...
            # compute current network loss and accuracy with respect to each individual instance, 
            if type(ins[-1]) is float:
                # do not slice the training phase flag
                ins_batch = slice_X(ins[:-1], q) + [ins[-1]]
            else:
                ins_batch = slice_X(ins, q)
            self._make_test_function()
            ff = self.test_function
            outs_epoch = self._test_train_spacedRep_loop(ff, ins_batch)
            los = outs_epoch[0]
            acs = outs_epoch[1]
                    
            # rearrange instances in Leitner queues 
            mask1, mask2 = acs >= acc_thr, acs < acc_thr
            promotes = q[mask1].tolist()
            demotes = q[mask2].tolist()
            for i in range(len(allQs) - 1, -1, -1):
                if ((epoch + 1) % int(pow(space_pow, i))) == 0:
                    q_set = set(allQs[i])
                    if i < len(allQs) - 1:
                        p = [l for l in promotes if l in q_set]
                        allQs[i + 1].extend(p)
                        allQs[i] = []    
                    if i == len(allQs) - 1:
                        [allQs[i].remove(l) for l in demotes if l in q_set]
            allQs[0] = list(demotes)
             
            if len(q0_stat) == 0:
                q0_stat = collections.Counter([item for item in demotes])
            else:
                q0_stat = q0_stat + collections.Counter([item for item in demotes])
            
            # update spurious score for instances 
            Z = float(len(demotes))
            for item in demotes:
                if item in q0_stat:
                    q0_stat[item] += 1. / Z + los[np.where(q==item)[0][0]]
                else:                    
                    q0_stat[item] = 1. / Z + los[np.where(q==item)[0][0]]
            
            allQ_list = [len(allQs[i]) for i in range(0, len(allQs))]
            qAll_stat[epoch + 1] = [allQ_list , [sum(allQ_list)]]
        ...
        # create a ranked list of spurious instances using spurious scores 
        q0_sorted = [i for (i,_) in sorted(q0_stat.items(), key=lambda x:x[1], reverse=True)]         
        return self.history, qAll_stat, q0_sorted
    ...
    def _test_train_spacedRep_loop(self, f, ins, verbose=0):
        '''Abstract method to loop over some data in batches.
        ...
        if verbose == 1:
            progbar = Progbar(target=nb_sample)
        batches = make_batches(nb_sample, 1)# batch_size must be 1!
        index_array = np.arange(nb_sample)
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            batch_ids = index_array[batch_start:batch_end]
            if type(ins[-1]) is float:
    ...
    def fit(self, x, y, batch_size=32, nb_epoch=10, verbose=1, callbacks=[],
            ...
            class_weight=None, sample_weight=None, kern='', acc_thr=.8):
        ...
        return self._fit_loop_lit(f, ins, out_labels=out_labels,
                  batch_size=batch_size, nb_epoch=nb_epoch,
                  verbose=verbose, callbacks=callbacks,
                  val_f=val_f, val_ins=val_ins, shuffle=shuffle, acc_thr = acc_thr,
